...
import ch.dominikponniah.mediaplayer.regenbogen.MainActivity
import ch.dominikponniah.mediaplayer.regenbogen.MediaItemData
...
import ch.dominikponniah.mediaplayer.regenbogen.media.extensions.isPlayEnabled
import ch.dominikponniah.mediaplayer.regenbogen.media.extensions.isPlaying
import ch.dominikponniah.mediaplayer.regenbogen.media.extensions.isPrepared
import ch.dominikponniah.mediaplayer.regenbogen.utils.Event
...
class MainActivityViewModel(
    ...
    fun mediaItemClicked(clickedItem: MediaItemData) {
        if (clickedItem.browsable) {
            ...
            playMedia(clickedItem, pauseAllowed = true)
    ...
    fun showFragment(fragment: Fragment, backStack: Boolean = true, tag: String? = null) {
        _navigateToFragment.value = Event(
  ...
  fun pauseMedia(mediaItem: MediaItemData, pauseAllowed: Boolean = true) {
    val nowPlaying = musicServiceConnection.nowPlaying.value
    val transportControls = musicServiceConnection.transportControls
    ...
    if (isPrepared && mediaItem.mediaId == nowPlaying?.id) {
      musicServiceConnection.playbackState.value?.let { playbackState ->
        when {
          playbackState.isPlaying ->
            if (pauseAllowed) transportControls.stop() else Unit
          playbackState.isPlayEnabled -> transportControls.stop()
          else -> {
            Log.w(
                TAG, "Playable item clicked but neither play nor pause are enabled!" +
                " (mediaId=${mediaItem.mediaId})"
